...
impl ModTrait for Mod {
    ...
    fn handle_xmpp_message(
        ...
        match self {
            ...
            Mod::Omemo(r#mod) => r#mod.handle_xmpp_message(aparte, account, message, delay, archive),
        }
    }
}
...
impl AparteCore {
    pub fn new(config_path: PathBuf) -> Self {
        ...
        core.add_mod(Mod::UI(mods::ui::UIMod::new(&config)));
    ...
    pub fn init(&mut self) -> Result<(), ()> {
        ...
        for (_, r#mod) in mods.iter() {
            if let Err(err) = r#mod.write().unwrap().init(&mut aparte) {
                return Err(err);
            }
        }
