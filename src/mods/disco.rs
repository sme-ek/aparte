...
impl DiscoMod {
    pub fn new<C: Into<String>, T: Into<String>, L: Into<String>, N: Into<String>>(category: C, type_: T, lang: L, name: N) -> Self {
        Self {
    ...
    pub fn add_feature<S: Into<String>>(&mut self, feature: S) {
        ...
        debug!("Adding `{}` feature", feature.var);
    ...
    pub fn disco(&mut self, node: Option<String>) -> Element {
    ...
    fn on_event(&mut self, aparte: &mut Aparte, event: &Event) {
        match event {
            Event::Connected(account, _jid) => {
                ...
                aparte.send(account, self.disco(None));
            }
